...
Class DataCrenasFinder
{
    ...
    public function findDataCrenasByUserId($prmUserId, $prmGroupe = null)
    { 
        $queryBuilder = $this->em->createQueryBuilder();
        ...
        if (null != $prmGroupe) {
            ...
            ->from('App:DataCrenas', 'dc') 
                ->join('dc.User', 'u') 
                ->andWhere('u.id = :userId and dc.groupe =:groupe ') 
                ->setParameter('userId', $prmUserId)
                ->setParameter('groupe', $prmGroupe); 
        } else {
            $queryBuilder 
        ->select(['dc.id, dc.totalCRENASAnneePrecedent, dc.totalProjeteAnneePrecedent, dc.totalAnneeProjection, dc.resultatAnneePrecedent, dc.resultatAnneeProjection,
                    dc.besoinAPTE, dc.besoinAMOX, dc.besoinFichePatient, dc.besoinRegistre, dc.besoinCarnetRapportCRENAS, 
    ...
    public function findAllDataCrenasByRegionId($prmGroupeId)
    {
        $queryBuilder = $this->em->createQueryBuilder();
        $queryBuilder 
        ->select(['dc.id, dc.totalCRENASAnneePrecedent, dc.totalProjeteAnneePrecedent, dc.totalAnneeProjection, dc.resultatAnneePrecedent, dc.resultatAnneeProjection,
        ...
        ->from('App:DataCrenas', 'dc') 
            ...
            ->leftJoin('u.District', 'd')
            ->leftJoin('u.Province', 'p')
            ->andWhere('dc.groupe = :prmGroupeId')
    ...
    public function findDataCrenasByRegionId2($prmRegionId, $prmGroupeId)
    {
        $queryBuilder = $this->em->createQueryBuilder();
        $queryBuilder 
        ->select(['dc.id, dc.totalCRENASAnneePrecedent, dc.totalProjeteAnneePrecedent, dc.totalAnneeProjection, dc.resultatAnneePrecedent, dc.resultatAnneeProjection,
    ...
    public function findAllDataCrenasByGroupe($prmGroupeId)
    {
        $queryBuilder = $this->em->createQueryBuilder();
        $queryBuilder 
            ->select('g.id AS idGroupe, 
            ...
            if (is_array($result) && count($result) > 0) {  
                for ($i=0; $i < count($result); $i++) { 
                    $idRegion = $result[$i]["regionId"]; 
                    $dataCrenas = $this->findDataCrenasByRegionId2($idRegion, $prmGroupeId); 
                    if ($dataCrenas != null) {
                        if (!in_array($dataCrenas, $lstDataCrenas)) {
                            $lstDataCrenas[] = $dataCrenas;
                        }
                    } 
                } 
            }
            dd($result, $lstDataCrenas);
        return $lstDataCrenas;
    }

    public function findDataCrenasByGroupeByRegion($prmGroupeId = null, $idRegion = null)
    {
        $queryBuilder = $this->em->createQueryBuilder();
        $queryBuilder 
            ->select('g.id AS idGroupe, 
